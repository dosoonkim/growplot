...
if __name__ == '__main__':
    ...
    for k,df in df_dict.items():
        ...
        params, pcov = curve_fit(f, xdata, ydata, maxfev=8000)
        ...
        final_proto_df.append({'tag': k, 'max': params[0], 'max_sd': stdevs[0], 'max_slope': max_slope, 'max_slope_sd': max_slope_sd, 'lag_time': lag_time, 'lag_time_sd': lag_time_sd})
    
    finaldf = pd.DataFrame.from_dict(final_proto_df)
    finaldf.to_csv(args.filename[0].replace('.xlsx', '_params.csv'))
